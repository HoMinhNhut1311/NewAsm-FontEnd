...
export const createUser = async ({username, password, roleNames}) => {
    const respone = await request.post('user/', {
        username : username,
        password : password,
        roleNames : roleNames
    }).catch((error) => {
...
// Update Profile Api
export const UpdateProfile = async (profileId,fullname,sex,birth,email,phone,userId) => {
...
export const UploadImageProFile = async (profileId, formData) => {
    const respone = await request.post(`mediaFile/profile/${profileId}`,formData);
    return respone;
}
